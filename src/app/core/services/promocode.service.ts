...
export class PromocodeService {
  ...
  public getAllPromocode(usertoken: any, page:any, size:any) {
    return this._httpReqService.request({
      method: APP_CONSTANTS.API_METHODS.GET,
      url: `${this._configService.getApiUrl()}${environment.SERVICE_APIS.GET_ALL_PROMOCODE}?page=${page}&size=${size}`,
  ...
  public getSpecificPromocode(groupId: string, usertoken: string) {
    return this._httpReqService.request({
      method: APP_CONSTANTS.API_METHODS.GET,
      url: this._configService.getApiUrl()+environment.SERVICE_APIS.SPECIFIC_PROMOCODE+ '/' + groupId,
      headerConfig: { token: usertoken }
    }).pipe(
  ...
  public createPromocode(payload,usertoken) {


    return this._httpReqService.request({
      method: APP_CONSTANTS.API_METHODS.POST,
      url: this._configService.getApiUrl()+environment.SERVICE_APIS.ADD_PROMOCODE,
      body: payload,
      headerConfig: {token:usertoken}
    })
      .pipe(
  ...
  public updatePromocode(editId,requestBody:any,usertoken:any) {

    //console.log("Api",requestBody);

    const url = this._configService.getApiUrl()+environment.SERVICE_APIS.UPDATE_PROMOCODE+ '/' + editId;
    return this._httpReqService.request({
  ...
  public deletePromocode(deleteId,usertoken) {


    return this._httpReqService.request({
      method: APP_CONSTANTS.API_METHODS.DELETE,
      url: this._configService.getApiUrl()+environment.SERVICE_APIS.DELETE_PROMOCODE+ '/' + deleteId,
      headerConfig: {token:usertoken}
    })
      .pipe(
        map(response => this._extractResponse(response))
  ...
  public deleteMultipleData(requestBody:{promo_list:string[]},usertoken) {
    return this._httpReqService.request({
      method: APP_CONSTANTS.API_METHODS.POST,
      url: this._configService.getApiUrl()+environment.SERVICE_APIS.BULK_PROMOCODE_DELETE,
      body: requestBody,
      headerConfig: {token:usertoken}
    })
      .pipe(
