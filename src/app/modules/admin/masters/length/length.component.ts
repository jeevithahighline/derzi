...
export class LengthComponent {
  ...
  deleteSelected() {
    const selectedIds = this.lengths.filter(item => item.isSelected).map(item => item._id);
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        ...
        this._masterservice.deleteMultipleData(requestBody, this.usertoken).subscribe({
          next: () => {
            this._toastrService.showSuccess("Selected length deleted successfully");
            this.loadLength(); // refresh table
          },
          error: () => {
            this._toastrService.showError("Failed to delete selected length");
          }
        });
      } else {
        this._toastrService.showError("Deletion cancelled by user");
