...
@Component({
  ...
  templateUrl: './sizeform.component.html',
  styleUrl: './sizeform.component.scss'
})
...
export class SizeformComponent {
  ...
  constructor(
    private fb: FormBuilder,private _masterservice: SizeService,private _toastrService: ToastService,
  ...
  ngOnInit(): void {
    ...
    // Prefill if editing
    if (this.data?.size) {
      this.editId = this.data.size._id;   
      this.dynamicForm.patchValue(this.data.size);
    }
  }
  ...
  public saveData(data: any): Observable<any> {
   ...
   const insertData = {
    ...
    return this._masterservice.createSize(insertData, this.usertoken).pipe(
      tap(res => this.handleApiResponse(res, 'Size created successfully', 'Size creation failed')),
      catchError(error => this.handleError(error, 'Size is not created. Please contact administrator'))
    );   
  ...
  public updateData(data: any, editId: any): Observable<any> {    
    ...
    return this._masterservice.updateSize(this.editId,updatedInfo,this.usertoken).pipe(
      tap(res => this.handleApiResponse(res, 'Size updated successfully', 'Size updation failed')),
      catchError(error => this.handleError(error, 'Size is not created. Please contact administrator'))
    );   
