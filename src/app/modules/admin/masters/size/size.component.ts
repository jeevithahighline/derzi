...
export class SizeComponent {
  ...
  deleteSelected() {
    const selectedIds = this.sizes.filter(item => item.isSelected).map(item => item._id);
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        ...
        this._masterservice.deleteMultipleData(requestBody, this.usertoken).subscribe({
          next: () => {
            this._toastrService.showSuccess("Selected size deleted successfully");
            this.loadSize(); // refresh table
          },
          error: () => {
            this._toastrService.showError("Failed to delete selected size");
          }
        });
      } else {
        this._toastrService.showError("Deletion cancelled by user");
