...
export class ProductsComponent {
    ...
    if (token) {
      this._productservice.getAllProduct(token, this.page, this.pageSize).subscribe({
        next: (res: any) => {
          ...
          this.totalItems = res.data.total || 0;  // backend must return total count
  ...
  deleteSelected() {
    const selectedIds = this.products.filter(item => item.isSelected).map(item => item._id);
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        ...
        this._productservice.deleteMultipleData(requestBody, this.usertoken).subscribe({
          next: () => {
            this._toastrService.showSuccess("Selected products deleted successfully");
            this.loadProducts(); // refresh table
          },
          error: () => {
            this._toastrService.showError("Failed to delete selected products");
          }
        });
      } else {
        this._toastrService.showError("Deletion cancelled by user");
