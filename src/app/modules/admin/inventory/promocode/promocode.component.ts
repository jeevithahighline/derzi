...
export class PromocodeComponent {
  ...
  deleteSelected() {
    const selectedIds = this.promocode.filter(item => item.isSelected).map(item => item._id);
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        ...
        this._promocodeservice.deleteMultipleData(requestBody, this.usertoken).subscribe({
          next: () => {
            this._toastrService.showSuccess("Selected promocode deleted successfully");
            this.loadPromocode(); // refresh table
          },
          error: () => {
            this._toastrService.showError("Failed to delete selected promocode");
          }
        });
      } else {
        this._toastrService.showError("Deletion cancelled by user");
